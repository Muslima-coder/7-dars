function HomeIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 28 28"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_10271_113"
                    style={{ maskType: "alpha" }}
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="28"
                >
                    <rect width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_10271_113)">
                    <path
                        d="M26.4601 8.83515L14.671 2.47331C14.3245 2.28665 13.908 2.28665 13.5626 2.47331L1.77931 8.83515C1.21231 9.14315 1.00114 9.85013 1.30681 10.4171C1.51681 10.808 1.91931 11.0296 2.33348 11.0296C2.52015 11.0296 2.71148 10.9853 2.88765 10.8896L3.74398 10.4276L5.59898 23.5526C5.85098 24.969 7.12731 25.9583 8.70231 25.9583H19.5313C21.1063 25.9583 22.3826 24.969 22.637 23.5223L24.4896 10.4265L25.3495 10.8908C25.9153 11.1976 26.6235 10.9865 26.9291 10.4195C27.236 9.85247 27.0225 9.14431 26.4578 8.83864L26.4601 8.83515ZM14.1168 18.2676C12.0226 18.2676 10.3251 16.5701 10.3251 14.476C10.3251 12.3818 12.0226 10.6843 14.1168 10.6843C16.211 10.6843 17.9085 12.3818 17.9085 14.476C17.9085 16.5701 16.211 18.2676 14.1168 18.2676Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="29"
                viewBox="0 0 28 29"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_8_700"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="29"
                >
                    <rect y="0.768066" width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_8_700)">
                    <path
                        d="M26.0286 9.57058L14.2417 3.20641C13.9816 3.06641 13.6701 3.06641 13.4099 3.20641L1.62539 9.57058C1.20073 9.80041 1.04206 10.3312 1.2719 10.7571C1.4294 11.0487 1.73156 11.2156 2.0419 11.2156C2.1819 11.2156 2.32539 11.1806 2.45723 11.1106L3.40806 10.5972L5.30856 24.0291C5.55823 25.4466 6.83456 26.4347 8.40956 26.4347H19.2386C20.8159 26.4347 22.0911 25.4454 22.3454 23.9999L24.2424 10.5949L25.1967 11.1106C25.6214 11.3357 26.1534 11.1806 26.3832 10.7559C26.6119 10.3324 26.4532 9.80158 26.0286 9.57058ZM20.6176 23.7257C20.4927 24.4327 19.7974 24.6847 19.2409 24.6847H8.41189C7.85189 24.6847 7.15773 24.4327 7.03756 23.7537L5.05189 9.71057L13.8252 4.97158L22.6009 9.70824L20.6176 23.7257Z"
                        fill="black"
                    />
                    <path
                        d="M9.4152 14.9535C9.4152 17.3849 11.3927 19.3635 13.8252 19.3635C16.2577 19.3635 18.2352 17.3849 18.2352 14.9535C18.2352 12.5222 16.2577 10.5435 13.8252 10.5435C11.3927 10.5435 9.4152 12.5222 9.4152 14.9535ZM16.4852 14.9535C16.4852 16.4212 15.2929 17.6135 13.8252 17.6135C12.3575 17.6135 11.1652 16.4212 11.1652 14.9535C11.1652 13.4859 12.3575 12.2935 13.8252 12.2935C15.2929 12.2935 16.4852 13.4859 16.4852 14.9535Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function ExploreIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 20 21"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M18.5848 8.23606C19.2448 8.23606 19.7848 7.69806 19.7848 7.03606C19.7848 6.37406 19.2448 5.83605 18.5848 5.83605H15.3648L15.6748 2.28905C15.7018 1.97105 15.6048 1.65905 15.3978 1.41405C15.1918 1.16905 14.9028 1.01805 14.5758 0.989049C13.9258 0.954049 13.3408 1.42105 13.2828 2.08205L12.9568 5.83605H7.89981L8.20781 2.29105C8.26781 1.63305 7.77781 1.04905 7.11081 0.989049C6.44581 0.939049 5.87581 1.41906 5.81781 2.08106L5.49281 5.83506H2.16281C1.49981 5.83506 0.962814 6.37305 0.962814 7.03505C0.962814 7.69705 1.50081 8.23505 2.16281 8.23505H5.28482L4.84482 13.2991H1.41582C0.753817 13.2991 0.21582 13.8391 0.21582 14.4991C0.21582 15.1591 0.753817 15.7011 1.41582 15.7011H4.63582L4.32582 19.2491C4.26882 19.9061 4.75782 20.4891 5.41582 20.5491L5.52182 20.5541C6.14782 20.5541 6.66181 20.0821 6.71681 19.4561L7.04382 15.7031H12.0998L11.7918 19.2471C11.7318 19.9051 12.2218 20.4891 12.8818 20.5491L12.9878 20.5541C13.6048 20.5541 14.1298 20.0721 14.1828 19.4561L14.5078 15.7031H17.8378C18.4978 15.7031 19.0378 15.1631 19.0378 14.5031C19.0378 13.8431 18.4978 13.3011 17.8378 13.3011H14.7158L15.1558 8.23705H18.5858L18.5848 8.23606ZM12.7468 8.23606L12.3068 13.2991H7.25282L7.69281 8.23705H12.7478L12.7468 8.23606Z"
                    fill="black"
                />
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 28 28"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_10271_310"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="28"
                >
                    <rect width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_10271_310)">
                    <path
                        d="M24.2083 9.43374H19.6233L20.0573 4.44974C20.0993 3.96907 19.7423 3.54558 19.2616 3.50241C18.7751 3.46741 18.3562 3.81741 18.3142 4.29924L17.8674 9.43257H10.4941L10.9257 4.4509C10.9689 3.96906 10.6107 3.54558 10.1324 3.50241C9.64242 3.46741 9.22592 3.81741 9.18392 4.29924L8.73825 9.43257H4.12058C3.63758 9.43257 3.24558 9.82574 3.24558 10.3076C3.24558 10.7894 3.63758 11.1826 4.12058 11.1826H8.58659L7.94492 18.5641H3.20825C2.72525 18.5641 2.33325 18.9561 2.33325 19.4391C2.33325 19.9221 2.72525 20.3141 3.20825 20.3141H7.79325L7.35925 25.2981C7.31725 25.7787 7.67425 26.2022 8.15492 26.2454L8.23192 26.2489C8.68108 26.2489 9.06259 25.9047 9.10225 25.4486L9.54908 20.3152H16.9224L16.4908 25.2969C16.4488 25.7787 16.8058 26.2022 17.2864 26.2454L17.3634 26.2489C17.8126 26.2489 18.1941 25.9047 18.2337 25.4486L18.6794 20.3152H23.2959C23.7778 20.3152 24.1709 19.9221 24.1709 19.4402C24.1709 18.9584 23.7778 18.5652 23.2959 18.5652H18.8299L19.4716 11.1837H24.2083C24.6913 11.1837 25.0833 10.7917 25.0833 10.3087C25.0833 9.82574 24.6913 9.43374 24.2083 9.43374ZM17.0741 18.5641H9.70075L10.3424 11.1837H17.7157L17.0741 18.5641Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function NotificationIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 20 22"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M19.6968 15.2361C19.6768 15.2201 17.5568 13.5961 17.5938 9.20607C17.6138 6.67307 16.7818 4.42407 15.2468 2.87207C13.8718 1.47907 12.0098 0.708073 10.0048 0.700073H9.99179C7.98779 0.708073 6.12579 1.48007 4.74979 2.87207C3.21579 4.42507 2.3828 6.67406 2.4038 9.20506C2.4408 13.5371 0.383791 15.1721 0.301791 15.2351C0.0417911 15.4291 -0.0642083 15.7651 0.0367917 16.0731C0.137792 16.3811 0.426789 16.5881 0.749789 16.5881H5.24379C5.34379 19.1321 7.43179 21.1751 9.99979 21.1751C12.5678 21.1751 14.6548 19.1321 14.7558 16.5881H19.2498C19.5738 16.5881 19.8598 16.3801 19.9618 16.0731C20.0638 15.7661 19.9568 15.4291 19.6968 15.2361ZM9.99979 19.1761C8.53379 19.1761 7.34279 18.0291 7.24379 16.5881H12.7558C12.6558 18.0281 11.4658 19.1751 9.99979 19.1751V19.1761Z"
                    fill="black"
                />
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 28 28"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_10271_318"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="28"
                >
                    <rect width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_10271_318)">
                    <path
                        d="M25.314 19.2943C25.2907 19.2757 22.8173 17.381 22.8605 12.2593C22.8838 9.30533 21.9131 6.68032 20.1223 4.86848C18.5181 3.24332 16.3458 2.34498 14.0067 2.33331H13.9915C11.6535 2.34498 9.48115 3.24333 7.87581 4.86966C6.08615 6.68149 5.11315 9.30533 5.13882 12.2593C5.18199 17.311 2.78215 19.2208 2.68648 19.2943C2.38315 19.5195 2.25948 19.9127 2.37731 20.272C2.49631 20.6313 2.8323 20.8728 3.20797 20.8728H8.94799C9.06699 23.5678 11.2778 25.7262 13.9996 25.7262C16.7215 25.7262 18.93 23.5678 19.0478 20.8728H24.7901C25.1658 20.8728 25.5018 20.6325 25.6185 20.2732C25.7386 19.915 25.615 19.5206 25.3117 19.2955L25.314 19.2943ZM14.0008 23.9727C12.245 23.9727 10.8158 22.5995 10.7015 20.8717H17.3001C17.1835 22.5983 15.7566 23.975 14.0008 23.975V23.9727ZM5.11082 19.1217C5.97415 17.801 6.91682 15.589 6.88882 12.243C6.86782 9.72298 7.64015 7.59732 9.12065 6.09815C10.3958 4.80665 12.1306 4.09148 14.0008 4.08331C15.871 4.09265 17.6023 4.80666 18.8775 6.09933C20.3591 7.59849 21.1326 9.72299 21.1116 12.2442C21.0836 15.5902 22.0275 17.8033 22.8908 19.1228H5.11082V19.1217Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function MessagesIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 20 19"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M9.55 8.85007C9.823 9.03207 10.177 9.03207 10.45 8.85007L20 2.48407V2.26807C20 1.02807 18.99 0.0180664 17.75 0.0180664H2.25C1.01 0.0180664 0 1.02807 0 2.26807V2.46507L9.55 8.85007Z"
                    fill="black"
                />
                <path
                    d="M11.26 10.0631C10.877 10.3181 10.44 10.4451 10 10.4451C9.56 10.4451 9.12301 10.3181 8.74001 10.0621L0 4.22009V15.8901C0 17.1301 1.01 18.1401 2.25 18.1401H17.75C18.99 18.1401 20 17.1301 20 15.8901V4.2381L11.26 10.0611V10.0631Z"
                    fill="black"
                />
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 28 28"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_10271_314"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="28"
                >
                    <rect width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_10271_314)">
                    <path
                        d="M22.4583 3.5H5.54159C3.77175 3.5 2.33325 4.93967 2.33325 6.71067V21.2882C2.33325 23.0592 3.77175 24.5 5.54159 24.5H22.4583C24.2281 24.5 25.6666 23.0592 25.6666 21.2882V6.71067C25.6666 4.93967 24.2281 3.5 22.4583 3.5ZM5.54159 5.25H22.4583C23.2633 5.25 23.9166 5.90333 23.9166 6.70833V7.54132L14.5249 13.8028C14.2064 14.0128 13.7946 14.0152 13.4749 13.8005L4.08325 7.54132V6.70833C4.08325 5.90333 4.73659 5.25 5.54159 5.25ZM22.4583 22.7477H5.54159C4.73659 22.7477 4.08325 22.0943 4.08325 21.2893V9.59233L12.5299 15.2273C12.9768 15.526 13.4889 15.6753 13.9999 15.6753C14.5133 15.6753 15.0231 15.526 15.4699 15.2285L23.9166 9.59349V21.2858C23.9166 22.0908 23.2633 22.7442 22.4583 22.7442V22.7477Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function BookmarksIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 18 21"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M16.8998 20.2681C16.6998 20.2681 16.5998 20.2681 16.4998 20.1681L8.9998 14.6681L1.4998 20.0681C1.2998 20.2681 0.999799 20.2681 0.699799 20.1681C0.499799 20.0681 0.299805 19.7681 0.299805 19.4681V2.3681C0.299805 1.1681 1.2998 0.168091 2.4998 0.168091H15.2998C16.4998 0.168091 17.4998 1.1681 17.4998 2.3681V19.4681C17.4998 19.7681 17.2998 19.9681 17.0998 20.1681C17.0998 20.2681 16.9998 20.2681 16.8998 20.2681Z"
                    fill="black"
                />
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 24 25"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_8_717"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="24"
                    height="25"
                >
                    <rect y="0.768066" width="24" height="24" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_8_717)">
                    <path
                        d="M19.55 22.9181C19.393 22.9181 19.238 22.8681 19.108 22.7741L11.65 17.3461L4.192 22.7761C3.964 22.9401 3.66199 22.9661 3.40999 22.8361C3.15999 22.7091 3 22.4511 3 22.1691V5.01807C3 3.77807 4.01 2.76807 5.25 2.76807H18.048C19.288 2.76807 20.298 3.77807 20.298 5.01807V22.1681C20.298 22.4501 20.14 22.7081 19.888 22.8361C19.782 22.8911 19.665 22.9181 19.548 22.9181H19.55ZM11.65 15.6681C11.805 15.6681 11.96 15.7161 12.09 15.8121L18.8 20.6951V5.01807C18.8 4.60607 18.463 4.26807 18.05 4.26807H5.25C4.837 4.26807 4.5 4.60607 4.5 5.01807V20.6951L11.21 15.8121C11.34 15.7161 11.495 15.6681 11.65 15.6681Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function ListIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 20 21"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M17.75 0.768066H2.25C1.013 0.768066 0 1.78107 0 3.01807V18.5181C0 19.7551 1.013 20.7681 2.25 20.7681H17.75C18.987 20.7681 20 19.7551 20 18.5181V3.01807C20 1.78107 18.987 0.768066 17.75 0.768066ZM9 15.5181H5C4.586 15.5181 4.25 15.1821 4.25 14.7681C4.25 14.3541 4.586 14.0181 5 14.0181H9C9.414 14.0181 9.75 14.3541 9.75 14.7681C9.75 15.1821 9.414 15.5181 9 15.5181ZM15 11.5181H5C4.586 11.5181 4.25 11.1821 4.25 10.7681C4.25 10.3541 4.586 10.0181 5 10.0181H15C15.414 10.0181 15.75 10.3541 15.75 10.7681C15.75 11.1821 15.414 11.5181 15 11.5181ZM15 7.51807H5C4.586 7.51807 4.25 7.18207 4.25 6.76807C4.25 6.35407 4.586 6.01807 5 6.01807H15C15.414 6.01807 15.75 6.35407 15.75 6.76807C15.75 7.18207 15.414 7.51807 15 7.51807Z"
                    fill="black"
                />
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="28"
                viewBox="0 0 24 25"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_8_720"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="24"
                    height="25"
                >
                    <rect y="0.768066" width="24" height="24" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_8_720)">
                    <path
                        d="M19.75 22.7681H4.25C3.01 22.7681 2 21.7581 2 20.5181V5.01807C2 3.77807 3.01 2.76807 4.25 2.76807H19.75C20.99 2.76807 22 3.77807 22 5.01807V20.5181C22 21.7581 20.99 22.7681 19.75 22.7681ZM4.25 4.26807C3.836 4.26807 3.5 4.60507 3.5 5.01807V20.5181C3.5 20.9311 3.836 21.2681 4.25 21.2681H19.75C20.164 21.2681 20.5 20.9311 20.5 20.5181V5.01807C20.5 4.60507 20.164 4.26807 19.75 4.26807H4.25Z"
                        fill="black"
                    />
                    <path
                        d="M16.75 9.26807H6.75C6.336 9.26807 6 8.93107 6 8.51807C6 8.10507 6.336 7.76807 6.75 7.76807H16.75C17.164 7.76807 17.5 8.10307 17.5 8.51807C17.5 8.93307 17.164 9.26807 16.75 9.26807ZM16.75 13.3781H6.75C6.336 13.3781 6 13.0421 6 12.6281C6 12.2141 6.336 11.8781 6.75 11.8781H16.75C17.164 11.8781 17.5 12.2141 17.5 12.6281C17.5 13.0421 17.164 13.3781 16.75 13.3781ZM11.75 17.4881H6.75C6.336 17.4881 6 17.1531 6 16.7381C6 16.3231 6.336 15.9881 6.75 15.9881H11.75C12.164 15.9881 12.5 16.3251 12.5 16.7381C12.5 17.1511 12.164 17.4881 11.75 17.4881Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function ProfileIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="29"
                viewBox="0 0 28 29"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    style={{ maskType: "alpha" }}
                    id="mask0_8_814"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="29"
                >
                    <rect y="0.768066" width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_8_814)">
                    <path
                        d="M13.9701 14.9559C12.3881 14.9559 10.6194 14.7809 9.49011 13.4906C8.54044 12.4056 8.23361 10.7279 8.55095 8.36658C8.99428 5.06958 11.0196 3.10141 13.9713 3.10141C16.9229 3.10141 18.9494 5.06958 19.3916 8.36658C19.7089 10.7279 19.4009 12.4032 18.4513 13.4906C17.3196 14.7809 15.5521 14.9547 13.9713 14.9547L13.9701 14.9559ZM20.7939 26.4476H7.14628C6.37278 26.4476 5.68795 26.1209 5.22128 25.5306C4.72895 24.9076 4.54928 24.0489 4.74295 23.2346C5.71595 19.1162 9.50994 16.2381 13.9689 16.2381C18.4279 16.2381 22.2219 19.1151 23.1973 23.2346C23.3886 24.0489 23.2089 24.9076 22.7166 25.5294C22.2499 26.1186 21.5674 26.4452 20.7939 26.4452V26.4476Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    } else {
        return (
            <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_15_733"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="24"
                    height="24"
                >
                    <rect width="24" height="24" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_15_733)">
                    <path
                        d="M11.351 12.16C12.706 12.16 14.223 12.01 15.191 10.904C16.005 9.974 16.269 8.536 15.997 6.512C15.617 3.687 13.88 2 11.351 2C8.82198 2 7.08498 3.687 6.70498 6.514C6.43298 8.536 6.69698 9.974 7.51098 10.904C8.47898 12.011 9.99598 12.16 11.351 12.16ZM8.19098 6.712C8.35298 5.512 8.97798 3.5 11.351 3.5C13.724 3.5 14.349 5.513 14.511 6.712C14.718 8.262 14.568 9.339 14.061 9.917C13.606 10.437 12.795 10.66 11.351 10.66C9.90698 10.66 9.09598 10.437 8.64098 9.917C8.13398 9.339 7.98398 8.261 8.19098 6.712ZM19.631 19.58C18.754 16.054 15.349 13.59 11.351 13.59C7.35298 13.59 3.94798 16.054 3.07098 19.58C2.89898 20.272 3.04298 20.98 3.46598 21.52C3.87398 22.04 4.50598 22.34 5.19898 22.34H17.503C18.196 22.34 18.828 22.04 19.236 21.52C19.66 20.98 19.803 20.273 19.63 19.58H19.631ZM18.055 20.596C17.929 20.756 17.739 20.842 17.503 20.842H5.19898C4.96398 20.842 4.77298 20.757 4.64698 20.596C4.50998 20.422 4.46698 20.184 4.52698 19.942C5.23698 17.087 8.04398 15.092 11.351 15.092C14.658 15.092 17.465 17.086 18.175 19.942C18.235 20.184 18.192 20.422 18.055 20.596Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function MoreIcon({ active }) {
    if (active) {
        return (
            <svg
                width="28"
                height="29"
                viewBox="0 0 28 29"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_8_820"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="29"
                >
                    <rect y="0.768066" width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_8_820)">
                    <path
                        d="M18.9582 12.4347C17.8323 12.4347 16.9165 13.3529 16.9165 14.4764C16.9165 15.5999 17.8312 16.5181 18.9582 16.5181C20.0828 16.5181 20.9998 15.6011 20.9998 14.4764C20.9998 13.3517 20.0828 12.4347 18.9582 12.4347ZM18.9582 15.3514C18.4752 15.3514 18.0832 14.9594 18.0832 14.4764C18.0832 13.9946 18.4763 13.6014 18.9582 13.6014C19.44 13.6014 19.8332 13.9934 19.8332 14.4764C19.8332 14.9582 19.4412 15.3514 18.9582 15.3514ZM13.7082 12.4347C12.5812 12.4347 11.6665 13.3529 11.6665 14.4764C11.6665 15.5999 12.5823 16.5181 13.7082 16.5181C14.834 16.5181 15.7498 15.6011 15.7498 14.4764C15.7498 13.3517 14.8352 12.4347 13.7082 12.4347ZM13.7082 15.3514C13.2252 15.3514 12.8332 14.9594 12.8332 14.4764C12.8332 13.9946 13.2263 13.6014 13.7082 13.6014C14.19 13.6014 14.5832 13.9934 14.5832 14.4764C14.5832 14.9582 14.1912 15.3514 13.7082 15.3514ZM8.45817 12.4347C7.33234 12.4347 6.4165 13.3529 6.4165 14.4764C6.4165 15.5999 7.33234 16.5181 8.45817 16.5181C9.58284 16.5181 10.4998 15.6011 10.4998 14.4764C10.4998 13.3517 9.58167 12.4347 8.45817 12.4347ZM8.45817 15.3514C7.97517 15.3514 7.58317 14.9594 7.58317 14.4764C7.58317 13.9946 7.97634 13.6014 8.45817 13.6014C8.94 13.6014 9.33317 13.9934 9.33317 14.4764C9.33317 14.9582 8.94117 15.3514 8.45817 15.3514Z"
                        fill="black"
                    />
                    <path
                        d="M13.7082 27.0181C6.79217 27.0181 1.1665 21.3924 1.1665 14.4764C1.1665 7.56039 6.79217 1.93472 13.7082 1.93472C20.6242 1.93472 26.2498 7.56039 26.2498 14.4764C26.2498 21.3924 20.6242 27.0181 13.7082 27.0181ZM13.7082 3.68472C7.75817 3.68472 2.9165 8.52639 2.9165 14.4764C2.9165 20.4264 7.75817 25.2681 13.7082 25.2681C19.6582 25.2681 24.4998 20.4264 24.4998 14.4764C24.4998 8.52639 19.6582 3.68472 13.7082 3.68472Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    } else {
        return (
            <svg
                width="28"
                height="29"
                viewBox="0 0 28 29"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <mask
                    id="mask0_8_820"
                    maskUnits="userSpaceOnUse"
                    x="0"
                    y="0"
                    width="28"
                    height="29"
                >
                    <rect y="0.768066" width="28" height="28" fill="#C4C4C4" />
                </mask>
                <g mask="url(#mask0_8_820)">
                    <path
                        d="M18.9582 12.4347C17.8323 12.4347 16.9165 13.3529 16.9165 14.4764C16.9165 15.5999 17.8312 16.5181 18.9582 16.5181C20.0828 16.5181 20.9998 15.6011 20.9998 14.4764C20.9998 13.3517 20.0828 12.4347 18.9582 12.4347ZM18.9582 15.3514C18.4752 15.3514 18.0832 14.9594 18.0832 14.4764C18.0832 13.9946 18.4763 13.6014 18.9582 13.6014C19.44 13.6014 19.8332 13.9934 19.8332 14.4764C19.8332 14.9582 19.4412 15.3514 18.9582 15.3514ZM13.7082 12.4347C12.5812 12.4347 11.6665 13.3529 11.6665 14.4764C11.6665 15.5999 12.5823 16.5181 13.7082 16.5181C14.834 16.5181 15.7498 15.6011 15.7498 14.4764C15.7498 13.3517 14.8352 12.4347 13.7082 12.4347ZM13.7082 15.3514C13.2252 15.3514 12.8332 14.9594 12.8332 14.4764C12.8332 13.9946 13.2263 13.6014 13.7082 13.6014C14.19 13.6014 14.5832 13.9934 14.5832 14.4764C14.5832 14.9582 14.1912 15.3514 13.7082 15.3514ZM8.45817 12.4347C7.33234 12.4347 6.4165 13.3529 6.4165 14.4764C6.4165 15.5999 7.33234 16.5181 8.45817 16.5181C9.58284 16.5181 10.4998 15.6011 10.4998 14.4764C10.4998 13.3517 9.58167 12.4347 8.45817 12.4347ZM8.45817 15.3514C7.97517 15.3514 7.58317 14.9594 7.58317 14.4764C7.58317 13.9946 7.97634 13.6014 8.45817 13.6014C8.94 13.6014 9.33317 13.9934 9.33317 14.4764C9.33317 14.9582 8.94117 15.3514 8.45817 15.3514Z"
                        fill="black"
                    />
                    <path
                        d="M13.7082 27.0181C6.79217 27.0181 1.1665 21.3924 1.1665 14.4764C1.1665 7.56039 6.79217 1.93472 13.7082 1.93472C20.6242 1.93472 26.2498 7.56039 26.2498 14.4764C26.2498 21.3924 20.6242 27.0181 13.7082 27.0181ZM13.7082 3.68472C7.75817 3.68472 2.9165 8.52639 2.9165 14.4764C2.9165 20.4264 7.75817 25.2681 13.7082 25.2681C19.6582 25.2681 24.4998 20.4264 24.4998 14.4764C24.4998 8.52639 19.6582 3.68472 13.7082 3.68472Z"
                        fill="black"
                    />
                </g>
            </svg>
        );
    }
}
function DotsIcon() {
    return (
        <svg
            width="17"
            height="4"
            viewBox="0 0 17 4"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M3.92308 2C3.92308 3.10457 3.04487 4 1.96154 4C0.878211 4 0 3.10457 0 2C0 0.895431 0.878211 0 1.96154 0C3.04487 0 3.92308 0.895431 3.92308 2Z"
                fill="black"
            />
            <path
                d="M10.4615 2C10.4615 3.10457 9.58333 4 8.5 4C7.41667 4 6.53846 3.10457 6.53846 2C6.53846 0.895431 7.41667 0 8.5 0C9.58333 0 10.4615 0.895431 10.4615 2Z"
                fill="black"
            />
            <path
                d="M17 2C17 3.10457 16.1218 4 15.0385 4C13.9551 4 13.0769 3.10457 13.0769 2C13.0769 0.895431 13.9551 0 15.0385 0C16.1218 0 17 0.895431 17 2Z"
                fill="black"
            />
        </svg>
    );
}
function ModeIcon() {
    return (
        <svg
            width="24"
            height="22"
            viewBox="0 0 24 22"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M22.772 9.50599L17.154 7.31399L14.994 0.813988C14.892 0.506988 14.604 0.299988 14.282 0.299988C13.96 0.299988 13.672 0.506988 13.57 0.812988L11.41 7.31299L5.79 9.50499C5.503 9.61699 5.313 9.89499 5.313 10.205C5.313 10.515 5.503 10.79 5.791 10.903L11.411 13.095L13.571 19.595C13.673 19.901 13.961 20.108 14.283 20.108C14.605 20.108 14.893 19.901 14.995 19.595L17.155 13.095L22.775 10.903C23.062 10.791 23.252 10.513 23.252 10.203C23.252 9.89299 23.062 9.61799 22.774 9.50599H22.772ZM16.282 11.826C16.074 11.906 15.912 12.076 15.842 12.286L14.282 16.981L12.722 12.288C12.652 12.078 12.492 11.908 12.284 11.826L8.129 10.206L12.283 8.58399C12.491 8.50399 12.653 8.33399 12.723 8.12199L14.283 3.42899L15.843 8.12299C15.913 8.33499 16.073 8.50499 16.281 8.58599L20.436 10.206L16.281 11.828L16.282 11.826ZM6.663 2.81199H4.783V1.04999C4.783 0.635988 4.446 0.299988 4.033 0.299988C3.62 0.299988 3.283 0.635988 3.283 1.04999V2.81199H1.5C1.086 2.81199 0.75 3.14799 0.75 3.56199C0.75 3.97599 1.086 4.31199 1.5 4.31199H3.282V6.07399C3.282 6.48799 3.618 6.82399 4.032 6.82399C4.446 6.82399 4.782 6.48799 4.782 6.07399V4.31199H6.662C7.077 4.31199 7.412 3.97599 7.412 3.56199C7.412 3.14799 7.077 2.81199 6.662 2.81199H6.663ZM9.198 18.434H8.098V17.418C8.098 17.004 7.763 16.668 7.348 16.668C6.933 16.668 6.598 17.004 6.598 17.418V18.434H5.57C5.156 18.434 4.82 18.77 4.82 19.184C4.82 19.598 5.156 19.934 5.57 19.934H6.6V20.95C6.6 21.364 6.935 21.7 7.35 21.7C7.765 21.7 8.1 21.364 8.1 20.95V19.934H9.198C9.612 19.934 9.948 19.598 9.948 19.184C9.948 18.77 9.612 18.434 9.198 18.434Z"
                fill="black"
            />
        </svg>
    );
}
function ChooseImgIcon() {
    return (
        <svg
            width="24"
            height="25"
            viewBox="0 0 24 25"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <mask
                id="mask0_11_1687"
                style={{ maskType: "alpha" }}
                maskUnits="userSpaceOnUse"
                x="0"
                y="0"
                width="24"
                height="25"
            >
                <rect y="0.628052" width="24" height="24" fill="#C4C4C4" />
            </mask>
            <g mask="url(#mask0_11_1687)">
                <path
                    d="M19.75 2.62805H4.25C3.01 2.62805 2 3.63805 2 4.87805V20.3781C2 21.6181 3.01 22.6281 4.25 22.6281H19.75C20.99 22.6281 22 21.6181 22 20.3781V4.87805C22 3.63805 20.99 2.62805 19.75 2.62805ZM4.25 4.12805H19.75C20.163 4.12805 20.5 4.46505 20.5 4.87805V14.5541L16.642 10.6961C16.502 10.5561 16.312 10.4761 16.112 10.4761H16.109C15.909 10.4761 15.716 10.5561 15.577 10.7001L11.26 15.0841L9.44701 13.2781C9.30701 13.1381 9.11701 13.0581 8.91701 13.0581C8.72401 13.0281 8.522 13.1381 8.382 13.2851L3.5 18.2701V4.87805C3.5 4.46505 3.837 4.12805 4.25 4.12805ZM3.506 20.4081L8.924 14.8741L15.206 21.1281H4.25C3.848 21.1281 3.523 20.8061 3.506 20.4081ZM19.75 21.1281H17.33L12.323 16.1411L16.115 12.291L20.5 16.6751V20.3781C20.5 20.7911 20.163 21.1281 19.75 21.1281Z"
                    fill="#1D9BF0"
                />
                <path
                    d="M8.86816 10.479C9.72016 10.479 10.4102 9.78872 10.4102 8.93702C10.4102 8.08542 9.72016 7.39502 8.86816 7.39502C8.01616 7.39502 7.32617 8.08542 7.32617 8.93702C7.32617 9.78872 8.01616 10.479 8.86816 10.479Z"
                    fill="#1D9BF0"
                />
            </g>
        </svg>
    );
}
function GifIcon() {
    return (
        <svg
            width="24"
            height="25"
            viewBox="0 0 24 25"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <mask
                id="mask0_11_1701"
                style={{ maskType: "alpha" }}
                maskUnits="userSpaceOnUse"
                x="0"
                y="0"
                width="24"
                height="25"
            >
                <rect y="0.628052" width="24" height="24" fill="#C4C4C4" />
            </mask>
            <g mask="url(#mask0_11_1701)">
                <path
                    d="M18.7499 11.1081V9.40808H14.3499V15.8081H16.0499V13.8081H18.0499V12.1081H16.0499V11.1081H18.7499ZM11.4499 9.40808H13.1499V15.8081H11.4499V9.40808ZM7.8499 11.0081C8.2499 11.0081 8.7499 11.2081 9.0499 11.5081L10.2499 10.5081C9.6499 9.80808 8.7499 9.40808 7.8499 9.40808C6.0499 9.40808 4.6499 10.8081 4.6499 12.6081C4.6499 14.4081 6.0499 15.8081 7.8499 15.8081C8.8499 15.8081 9.6499 15.4081 10.2499 14.7081V12.2081H7.4499V13.4081H8.6499V14.0081C8.4499 14.1081 8.1499 14.2081 7.8499 14.2081C6.9499 14.2081 6.2499 13.5081 6.2499 12.6081C6.2499 11.8081 6.9499 11.0081 7.8499 11.0081Z"
                    fill="#1D9BF0"
                />
                <path
                    d="M20.25 2.62805H3.25C2.01 2.62805 1 3.63505 1 4.87505V20.382C1 21.6201 2.01 22.628 3.25 22.628H20.25C21.49 22.628 22.5 21.6201 22.5 20.382V4.87505C22.5 3.63505 21.49 2.62805 20.25 2.62805ZM21 20.382C21 20.792 20.664 21.128 20.25 21.128H3.25C2.836 21.128 2.5 20.792 2.5 20.382V4.87505C2.5 4.46305 2.836 4.12805 3.25 4.12805H20.25C20.664 4.12805 21 4.46305 21 4.87505V20.382Z"
                    fill="#1D9BF0"
                />
            </g>
        </svg>
    );
}
function BornIcon() {
    return (
        <svg
            width="24"
            height="24"
            viewBox="0 0 18 21"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M4.75 10.083C4.336 10.083 4 9.747 4 9.333C4 6.393 6.243 4 9 4C9.414 4 9.75 4.336 9.75 4.75C9.75 5.164 9.414 5.5 9 5.5C7.07 5.5 5.5 7.22 5.5 9.333C5.5 9.747 5.164 10.083 4.75 10.083Z"
                fill="black"
            />
            <path
                d="M17.75 9.333C17.75 4.323 13.825 0.25 9 0.25C4.175 0.25 0.25 4.324 0.25 9.333C0.25 13.938 3.57 17.745 7.855 18.33L6.155 20.16C6.018 20.305 5.982 20.517 6.062 20.7C6.142 20.882 6.322 21 6.522 21H11.479C11.677 21 11.857 20.882 11.936 20.7C12.016 20.517 11.98 20.305 11.844 20.16L10.144 18.33C14.429 17.745 17.749 13.938 17.749 9.333H17.75ZM9 16.917C5.002 16.917 1.75 13.515 1.75 9.333C1.75 5.151 5.002 1.75 9 1.75C12.998 1.75 16.25 5.15 16.25 9.333C16.25 13.516 12.998 16.917 9 16.917Z"
                fill="black"
            />
        </svg>
    );
}
function VectorIcon() {
    return (
       <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M18.222 7.18005H16.888C16.903 7.09005 16.916 6.99805 16.916 6.90305V4.59005C16.916 3.61005 16.119 2.81305 15.138 2.81305H1.5V1.37805C1.5 0.964052 1.164 0.628052 0.75 0.628052C0.336 0.628052 0 0.964052 0 1.37805V18.8501C0 19.2651 0.336 19.6001 0.75 19.6001C1.164 19.6001 1.5 19.2651 1.5 18.8501V17.416H12.056C13.036 17.416 13.834 16.619 13.834 15.639V13.326C13.834 13.231 13.82 13.139 13.806 13.048H18.223C19.203 13.048 20.001 12.25 20.001 11.2701V8.96005C20.001 7.97705 19.204 7.18005 18.223 7.18005H18.222ZM15.14 4.31305C15.292 4.31305 15.417 4.43705 15.417 4.59005V6.90005C15.417 7.05405 15.292 7.18005 15.139 7.18005H1.5V4.31005H15.14V4.31305ZM12.333 13.327V15.639C12.333 15.792 12.208 15.916 12.055 15.916H1.5V13.048H12.056C12.209 13.048 12.333 13.174 12.333 13.328V13.327ZM18.5 11.2701C18.5 11.423 18.375 11.5471 18.222 11.5471H1.5V8.68005H18.222C18.375 8.68005 18.5 8.80405 18.5 8.95705V11.2701Z" fill="#1D9BF0"/>
</svg>
    );
}
function SmileIcon() {
    return (
     <svg width="22" height="23" viewBox="0 0 22 23" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M10.75 22.1281C4.822 22.1281 0 17.3061 0 11.3781C0 5.45005 4.822 0.628052 10.75 0.628052C16.678 0.628052 21.5 5.45005 21.5 11.3781C21.5 17.3061 16.678 22.1281 10.75 22.1281ZM10.75 2.12805C5.65 2.12805 1.5 6.27805 1.5 11.3781C1.5 16.4781 5.65 20.6281 10.75 20.6281C15.85 20.6281 20 16.4781 20 11.3781C20 6.27805 15.85 2.12805 10.75 2.12805Z" fill="#1D9BF0"/>
<path d="M10.7502 16.4931C8.85824 16.4931 7.11725 15.5431 6.09425 13.9491C5.87025 13.6011 5.97125 13.1391 6.32025 12.9141C6.66825 12.6881 7.13225 12.7901 7.35625 13.1401C8.10325 14.3021 9.37224 14.9951 10.7512 14.9951C12.1302 14.9951 13.3992 14.3021 14.1472 13.1411C14.3712 12.7911 14.8352 12.6911 15.1832 12.9161C15.5332 13.1401 15.6332 13.6041 15.4092 13.9521C14.3842 15.5461 12.6432 16.4971 10.7512 16.4971L10.7502 16.4931Z" fill="#1D9BF0"/>
<path d="M13.4878 10.314C14.3041 10.314 14.9658 9.65233 14.9658 8.83603C14.9658 8.01973 14.3041 7.35803 13.4878 7.35803C12.6715 7.35803 12.0098 8.01973 12.0098 8.83603C12.0098 9.65233 12.6715 10.314 13.4878 10.314Z" fill="#1D9BF0"/>
<path d="M8.01218 10.314C8.82848 10.314 9.49018 9.65233 9.49018 8.83603C9.49018 8.01973 8.82848 7.35803 8.01218 7.35803C7.1959 7.35803 6.53418 8.01973 6.53418 8.83603C6.53418 9.65233 7.1959 10.314 8.01218 10.314Z" fill="#1D9BF0"/>
</svg>
    );
}


export {
    HomeIcon,
    ExploreIcon,
    NotificationIcon,
    MessagesIcon,
    BookmarksIcon,
    ListIcon,
    ProfileIcon,
    MoreIcon,
    DotsIcon,
    ModeIcon,
    ChooseImgIcon,
    GifIcon,
    BornIcon,
    VectorIcon,
    SmileIcon
};
